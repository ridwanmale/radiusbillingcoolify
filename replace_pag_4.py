...
def process_file(filepath):
    ...
    matches = list(re.finditer(r'<button[\s\S]*?>\s*Prev\s*</button>', content, re.IGNORECASE))
    
    for match in reversed(matches):
        prev_btn_start = match.start()
        prev_btn_end = match.end()
        
        next_btn_match = re.search(r'<button[\s\S]*?>\s*Next\s*</button>', content[prev_btn_end:], re.IGNORECASE)
