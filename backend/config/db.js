...
const checkPrimaryStatus = async () => {
  ...
  try {
    const conn = await mysql.createConnection({
      ...primaryConfig,
      connectTimeout: 5000 // 5 seconds timeout for quick check
    });
    await conn.ping();
    await conn.end();
...
const failoverPoolProxy = {
  ...
  // Get current active database status for API monitoring
  getStatus: function () {
    return {
      isPrimaryAlive: isPrimaryAlive,
      primaryHost: primaryConfig.host,
      backupHost: backupConfig.host,
      backupActive: !!backupPool
    };
  }
};
